...
* ``firedrake-install.log`` from Firedrake
* ``configure.log`` and ``make.log`` from PETSc

Likewise, if it's ``firedrake-update`` that fails, please include the
file ``firedrake-update.log``. The PETsc log files live in the
directory ``src/petsc/`` inside the directory where the Firedrake
