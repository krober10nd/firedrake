...
class MeshTopology(AbstractMeshTopology):
    ...
    def __init__(self, plex, name, reorder, distribution_parameters):
        ...
        label_boundary = (self._comm.size == 1) or distribute
        dmcommon.label_facets(plex, label_boundary=label_boundary)

        # Distribute the dm to all ranks
        if self._comm.size > 1 and distribute:
            ...
            try:
                ...
                partitioner.setShellPartition(self._comm.size, sizes, points)
            except TypeError:
                pass
...
def VertexOnlyMesh(mesh, vertexcoords):
    ...
    swarm = _pic_swarm_in_plex(mesh.topology._topology_dm, vertexcoords, fields=[("parentcellnum", 1, IntType), ("refcoord", tdim, RealType)])
